...
func (p *Parser) Parse(st *tp.StructLike) (map[*tp.Field]*Validation, error) {
	...
	for _, f := range st.GetFields() {
		v, err := p.parseField(st, f.Type, f.GetAnnotations())
		if err != nil {
...
func (p *Parser) parseField(st *tp.StructLike, typ *tp.Type, annotations []*tp.Annotation) (*Validation, error) {
	sym, err := p.cu.ResolveSymbolInRootScope(typ)
	if err != nil {
		return nil, err
	}
	if ok, _ := p.cu.IsEnumType(typ); ok {
	...
	default:
		return nil, fmt.Errorf("type %s not recognized", sym.TypeID)
	}
}
...
func (p *Parser) parseMap(st *tp.StructLike, keyType *tp.Type, valType *tp.Type, annotations []*tp.Annotation) (*Validation, error) {
